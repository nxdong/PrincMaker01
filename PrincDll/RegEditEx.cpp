...
{   
	...
	wchar_t*  PointStr;  
	//wchar_t   KeyName[32],ValueSz[MAX_PATH],ValueTemp[MAX_PATH];	
	//wchar_t*   KeyName,ValueSz,ValueTemp;	
	CString   KeyName,ValueSz,ValueTemp;
	...
	//	 SetKeySecurityEx(MainKey,SubKey,KEY_ALL_ACCESS);
		...
		case 0:
			...
			case REG_EXPAND_SZ:				 
				...
				if(RegQueryValueEx(hKey,Vname,NULL,&Type,(LPBYTE)ValueSz.GetBuffer(),&szSize) == ERROR_SUCCESS)
				{
					//strcpy(szData,DelSpace(ValueSz));
					szData == DelSpace(ValueSz.GetBuffer());
					iResult =1;
				}
				break;
			case REG_MULTI_SZ:	
				...
				if(RegQueryValueEx(hKey,Vname,NULL,&Type,(LPBYTE)ValueSz.GetBuffer(),&szSize) == ERROR_SUCCESS)
				{
// 					for(PointStr = ValueSz.GetBuffer(); *PointStr; PointStr =  strchr(PointStr,0)+1)
// 					{
// 					
...
// 					}
		...
		case 1:
			...
			//	memset(ValueSz,0,sizeof(ValueSz));
				szSize   = sizeof(ValueSz);
                if(RegEnumKeyEx(hKey,dwIndex++,ValueSz.GetBuffer(),&szSize,NULL,NULL,NULL,NULL) != ERROR_SUCCESS)
					break;
                //wsprintf(ValueTemp,"[%s]\r\n",ValueSz);
				ValueTemp.Format(_T("[%s]\r\n"),ValueSz);
			
				wcscat(szData,ValueTemp);
				iResult =1;
			}			 
			break;
		case 2:			  
			...
			{				 
                ...
                szSize = sizeof(ValueSz);
                if(RegEnumValue(hKey,dwIndex++,KeyName.GetBuffer(),&KnSize,NULL,&Type,(LPBYTE)ValueSz.GetBuffer(),&szSize) != ERROR_SUCCESS)
					break;
				switch(Type)				 				
				{				     
				case REG_SZ:					 						 
					wsprintf(ValueTemp.GetBuffer(),_T("%-24s %-15s %s \r\n"),KeyName,_T("REG_SZ"),ValueSz);					     
					break;
				case REG_EXPAND_SZ:                   						 
					wsprintf(ValueTemp.GetBuffer(),_T("%-24s %-15s %s \r\n"),KeyName,_T("REG_EXPAND_SZ"),ValueSz);
					break;
				case REG_DWORD:
					wsprintf(ValueTemp.GetBuffer(),_T("%-24s %-15s 0x%x(%d) \r\n"),KeyName,_T("REG_DWORD"),ValueSz,_ttoi(ValueSz));
					break;
				case REG_MULTI_SZ:
                    wsprintf(ValueTemp.GetBuffer(),_T("%-24s %-15s \r\n"),KeyName,_T("REG_MULTI_SZ"));
					break;
			    case REG_BINARY:
...
int WriteRegEx(HKEY MainKey,LPCTSTR SubKey,LPCTSTR Vname,DWORD Type,wchar_t* szData,DWORD dwData,int Mode)
{
	...
	//	SetKeySecurityEx(MainKey,Subkey,KEY_ALL_ACCESS);
		...
		case 1:	
			...
			case REG_SZ:		 
			case REG_EXPAND_SZ:			 			 
				if(RegSetValueEx(hKey,Vname,0,Type,(LPBYTE)szData,wcslen(szData)+1) == ERROR_SUCCESS) 				 
					iResult =1;				 			
				break;
		    case REG_DWORD:
